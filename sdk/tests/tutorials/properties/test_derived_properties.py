...
class DerivedPropertyTests(asynctest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    async def test_derived_property(self):
        ...
        self.upsert_ratings_property("BBG005SZG253", fitch_value=10, moodys_value=5)
        self.upsert_ratings_property("BBG00J50FHL5")
        self.upsert_ratings_property("BBG001LSHYT8", moodys_value=5)
        self.upsert_ratings_property("BBG00YG2ZBC7", fitch_value=10)
        ...
        moodys_then_fitch = self.get_instruments_with_derived_prop("BBG005SZG253")
        self.assertEqual(moodys_then_fitch, 5.0)

        # test case for derived property with no ratings
        no_ratings = self.get_instruments_with_derived_prop("BBG00J50FHL5")
        self.assertEqual(no_ratings, 0.0)

        # # test case for derived property with fitch only
        fitch_only = self.get_instruments_with_derived_prop("BBG00YG2ZBC7")
        self.assertEqual(fitch_only, 10.0)

        # test case for derived property with moodys only
        moodys_only = self.get_instruments_with_derived_prop("BBG001LSHYT8")
